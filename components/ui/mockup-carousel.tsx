...
function BrowserChrome({ site }: { site: SiteConfig }) {
  return (
    <div className="bg-[#DEE1E6] select-none">
      {/* Tab strip */}
      <div className="flex items-end px-2 pt-2 gap-0.5">
        <div className="flex items-center gap-2 bg-white rounded-t-lg px-3 py-1.5 max-w-[220px] border-t border-x border-[#C0C4CC]">
          <div
            className="w-3.5 h-3.5 rounded-sm shrink-0"
            style={{ backgroundColor: site.faviconColor }}
          />
          <span className="text-[11px] text-[#202124] truncate leading-none">
            {site.tab}
          </span>
      ...
      {/* Address bar */}
      <div className="flex items-center gap-1.5 px-3 py-2 bg-[#F1F3F4] border-t border-[#C0C4CC]">
        <span className="w-6 h-6 flex items-center justify-center text-[#5F6368] text-sm">‹</span>
        <span className="w-6 h-6 flex items-center justify-center text-[#BDC1C6] text-sm">›</span>
        <span className="w-6 h-6 flex items-center justify-center text-[#5F6368] text-sm">↻</span>
        <div className="flex-1 bg-white rounded-full py-1.5 px-4 flex items-center gap-2">
          <span className="text-[10px] text-[#188038]">🔒</span>
          <span className="text-[11px] text-[#202124]">{site.url}</span>
        </div>
        <span className="w-6 h-6 flex items-center justify-center text-[#5F6368] text-lg">⋮</span>
      </div>
    </div>
  );
}
...
// ── Single site slide ─────────────────────────────────────────────────────────
function SiteSlide({ site }: { site: SiteConfig }) {
  return (
    <div className="bg-white">
      ...
      <div
        ...
        style={{ backgroundColor: '#F7F7FA', borderColor: '#E6E6EC' }}
      >
        {[
          ...
          <div key={i} className="p-4" style={{ borderColor: '#E6E6EC' }}>
            <div className="w-2 h-2 rounded-full mb-2" style={{ backgroundColor: site.accentColor }} />
            <div style={{ fontWeight: 600, color: '#1C1B29' }}>{label}</div>
            <div style={{ marginTop: '2px', color: '#55555F' }}>{sub}</div>
          </div>
        ))}
      </div>
    </div>
