...
export default function ServicesSection() {
  ...
  const packages = [
    {
      key: 'start',
      name: t('packages.start.name'),
      subtitle: t('packages.start.subtitle'),
      desc: t('packages.start.desc'),
      price: t('packages.start.price'),
      featured: false,
    },
    {
      key: 'business',
      ...
      desc: t('packages.business.desc'),
      price: t('packages.business.price'),
      featured: true,
    },
    {
      key: 'shop',
      name: t('packages.shop.name'),
      subtitle: t('packages.shop.subtitle'),
      desc: t('packages.shop.desc'),
      price: t('packages.shop.price'),
      featured: false,
    },
  ];
  ...
  const careOptions = [
    {
      key: 'basic',
      name: t('care.basic.name'),
      subtitle: t('care.basic.subtitle'),
      desc: t('care.basic.desc'),
      price: t('care.basic.price'),
    },
    {
      key: 'plus',
      name: t('care.plus.name'),
      subtitle: t('care.plus.subtitle'),
      desc: t('care.plus.desc'),
      price: t('care.plus.price'),
    },
  ];
  ...
  return (
    <section className="py-24 bg-surface-2 dark:bg-transparent">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        {/* One-time label — mb-12 ensures the featured card's badge (-top-4)
            plus its sm:-translate-y-4 elevation (total -32px) stays clear */}
        <p className="text-xs font-semibold uppercase tracking-widest text-text-muted mb-12 text-center">
          {t('oneTime')}
        </p>

        {/* 3 packages — Business is elevated */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-end mb-16 pb-4">
          {packages.map((pkg) => (
            ...
            >
              ...
              <p className={`text-2xl font-black mb-5 ${pkg.featured ? 'text-white' : 'text-ink'}`}>
                {pkg.name}
              </p>

              {/* Price */}
              <p className={`text-3xl font-bold leading-none mb-1 ${pkg.featured ? 'text-white' : 'text-ink'}`}>
                {pkg.price}
              </p>
              ...
              <Link
                ...
                className={`inline-flex items-center justify-center px-4 py-3 rounded-[10px] text-sm font-semibold transition-all duration-200 active:scale-[0.98] ${
                  pkg.featured
                    ...
                    : 'border border-border text-ink hover:border-indigo/40 hover:text-indigo'
                }`}
              >
                {t('ctaQuote')}
              </Link>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-3 mb-0.5 flex-wrap">
                    <p className="font-bold text-ink">{care.name}</p>
