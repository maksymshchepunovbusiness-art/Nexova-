...
const INPUT =
  'w-full px-4 py-3 rounded-[8px] border border-border bg-surface-2 text-ink placeholder:text-text-muted focus-visible:outline-2 focus-visible:outline-indigo focus-visible:outline-offset-2 transition-colors duration-150';
...
export default function ContactForm({ locale }: Props) {
  ...
  if (state === 'success') {
    return (
      <div className="bg-surface-2 border border-border rounded-2xl p-8 sm:p-10 text-center">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-success/10 dark:bg-success/20 mb-6">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden
            className="stroke-success">
            <polyline points="20 6 9 17 4 12" />
          </svg>
        </div>
        <h2 className="text-h3 text-ink dark:text-white mb-3">{t('success.title')}</h2>
        <p className="text-body text-text-muted dark:text-white/75 mx-auto mb-6">
          {t('success.body')}
        </p>
        <button
          onClick={() => {
    ...
    <div className="bg-surface-2 border border-border shadow-md rounded-2xl p-6 sm:p-8">
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
        ...
        <div>
          ...
          />
          {errors.name && (
            <p id="cf-name-err" className="text-sm text-red-500 dark:text-red-400 mt-1.5">
              {errors.name}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.email && (
            <p id="cf-email-err" className="text-sm text-red-500 dark:text-red-400 mt-1.5">
              {errors.email}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.message && (
            <p id="cf-message-err" className="text-sm text-red-500 dark:text-red-400 mt-1.5">
              {errors.message}
            </p>
          )}
        </div>
        ...
        {state === 'error' && (
          <div
            role="alert"
            className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-[8px] px-4 py-3"
          >
            {t('errors.generic')}
          </div>
        )}
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 self-start px-7 py-3.5 rounded-[8px] bg-terracotta text-white font-medium hover:bg-terracotta/90 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-200 shadow-sm"
        >
          {state === 'loading' ? (
            <>
